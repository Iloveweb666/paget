 ...
 */
import type { LLM, Message } from '@paget/llm';
import { ToolRegistry } from '../tools/tool.registry';
import { buildMacroToolSchema } from './macro-tool.chain';
...
export interface AgentChainInput {
  ...
  // 允许批量执行的工具名列表，为空则每步只能 1 个 action / Batchable tool names, empty = single action per step
...
export async function executeAgentStep(
  ...
  // 构建宏工具 Zod 模式（根据 batchableTools 控制是否允许批量）/ Build macro tool Zod schema (batch controlled by batchableTools)
  const macroToolSchema = buildMacroToolSchema(registry, { batchableTools: input.batchableTools });
  ...
  // 定义宏工具 — 使用 @paget/llm Tool 接口 / Define the macro tool using @paget/llm Tool interface
  const tools = {
    agent_step: {
      ...
      execute: async (args: AgentChainOutput) => args,
    },
  };

  // 调用 LLM / Call LLM
  const abortController = new AbortController();
  const result = await llm.invoke(messages, tools, abortController.signal, {
    toolChoiceName: 'agent_step',
  });

  const args = result.toolCall.args as AgentChainOutput;
