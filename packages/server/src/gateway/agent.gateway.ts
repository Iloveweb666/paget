...
import type { PromiseResolver, PageStateResponse, BatchResultResponse } from './gateway.types';
...
export class AgentGateway implements OnGatewayConnection, OnGatewayDisconnect {
   ...
   * Per-session pending page state / batch result Promise resolvers
   */
  private pageStateResolvers = new Map<string, PromiseResolver<PageStateResponse>>();
  private batchResultResolvers = new Map<string, PromiseResolver<BatchResultResponse>>();
  ...
  async handleTaskSubmit(
    ...
    try {
      // 意图分类与路由 / Intent classification and routing
      const intent = await this.intentRouter.classifyIntent(task);
      const route = await this.intentRouter.route(intent);
      this.logger.log(`Intent classified: "${task}" → ${intent.type} (confidence: ${intent.confidence}), route: ${route.routeTo}`);

      if (route.routeTo === 'direct_chat') {
      ...
      // 自动化意图 → 走 agent 循环 / Automation intent → agent loop
  ...
  handleTaskCancel(
    ...
    client.emit(WS_EVENTS.AGENT_STATUS, { status: 'idle', sessionId: data.sessionId });
  }
  ...
  handleBatchResult(
    @MessageBody() data: { sessionId: string; results: BatchResultResponse },
  ) {
