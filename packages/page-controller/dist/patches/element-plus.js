 ...
 */
export async function handleElSelect(trigger, optionText) {
 ...
 */
export async function handleElDatePicker(trigger, dateStr) {
    // 点击触发元素打开日期面板 / Click to open date panel
    const input = trigger.querySelector(".el-input__inner");
    if (!input)
        return false;
    input.dispatchEvent(new MouseEvent("click", { bubbles: true }));
