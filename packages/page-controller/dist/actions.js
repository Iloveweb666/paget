 ...
 */
export async function movePointerToElement(element) {
 ...
 */
export async function inputText(element, text) {
 ...
 */
export async function selectOption(element, optionText) {
 ...
 */
export async function scrollVertically(down, scrollAmount, element) {
    ...
    if (!canScroll(el)) {
        el =
            Array.from(document.querySelectorAll("*")).find((e) => canScroll(e)) ||
 ...
 */
export async function scrollHorizontally(right, scrollAmount, element) {
 ...
 */
export async function executeAction(action, selectorMap) {
    try {
        ...
        switch (tool) {
            // 点击操作 / Click action
            case "click": {
                const el = getElement(params.index);
            ...
            case "input": {
                const el = getElement(params.index);
                await inputText(el, params.text);
                return {
                    action,
                    success: true,
            ...
            case "select": {
                const el = getElement(params.index);
                await selectOption(el, (params.value || params.optionText));
                return {
                    action,
                    success: true,
            ...
            case "scroll": {
                const el = params.index != null ? getElement(params.index) : null;
            ...
            case "scroll_horizontally": {
                ...
                const amount = params.amount || params.pixels || 300;
            ...
            case "execute_javascript": {
                ...
                const asyncFn = eval(`(async () => { ${params.code} })`);
            ...
            case "done": {
                return {
                    action,
                    success: true,
                    output: params.data || "Task completed",
                };
            }
 ...
 */
export async function executeBatch(actions, selectorMap, onProgress) {
    const results = [];
