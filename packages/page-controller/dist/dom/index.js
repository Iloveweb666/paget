 ...
 */
export function getFlatTree(root = document.body, config = {}) {
 ...
 */
export function flatTreeToString(nodes) {
    return nodes
 ...
 */
export function getSelectorMap(root = document.body, nodes) {
    const map = new Map();
    ...
    for (const el of allElements) {
        if (nodeIdx >= interactiveNodes.length)
            break;
        if (isInteractive(el)) {
...
function isInteractive(element) {
    ...
    if (element.getAttribute('onclick') || element.getAttribute('tabindex'))
        return true;
    return false;
}
/**
